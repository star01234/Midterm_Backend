...
const isMod = async (req, res, next) => {
  try {
    ...
    return res.status(403).send({ message: "Unauthorized access, require Moderator Role!" });
  } catch (error) {
    return res.status(500).send({ message: error.message });
  }
};
...
const isModOrAdmin = async (req, res, next) => {
  try {
    ...
    return res.status(403).send({ message: "Unauthorized access, require Mod or Admin Role!" });
  } catch (error) {
    return res.status(500).send({ message: error.message });
  }
};
